...
import com.Gamesareme.TCO.entity.mobs.enemies.MobJumper;
...
public class Jumper extends MobJumper{
	
	private static SpriteSheet sheet = new SpriteSheet("Jumper.png");
	
	public int health = 1;

	public Jumper(int x, int y, int time, World world) {
		super(x, y, world);
		setHealth(1);
		sprite = new Sprite(1, 3, 32, sheet);
		sprite2 = new Sprite(1, 2, 32, sheet);
		this.jumFrequ = time * 60;
	}
	...
	public void tick() {
		...
		if(this.currentJumpTime <= 0){
			this.velY -= this.jumpHieght;
			this.jumping = true;
			this.currentJumpTime = jumFrequ;
		}
		super.tick();
	}
